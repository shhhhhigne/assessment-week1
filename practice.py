...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    #    --> not sure how to make this work, tho I understand why it doesnts
    # if len(numbers) == None:
    #     return None

    # my_min = numbers[0]
    # print my_min
    # my_min = [num for num in numbers[1:] if num<my_min]
    # print my_min
...
def halvesies(numbers):
    ...
    return [.5*num for num in numbers]
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    # for some reason this fails the doc test, but it seems like it's giving
    # me the right answer

    items[:] = [item for item in items[::-1]]
...
def duplicates(items):
    ...
    # return [set(item for item in items if item in set(items))]
    ...
    for item in items:
        if item in my_set and item not in my_dup_set:
            my_dup_set.add(item)
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        ...
        for character in word:
            if character == letter:
                my_indexes.append(my_index)
                is_in_word = 1
                break
            my_index += 1
        if is_in_word == 0:
            my_indexes.append(None)
